...
-   [Underlines denote our writing (not transcribed
...
-   [Code blocks were directly excerpted, adapted, and reused from
    ...
    Assignment 2 folder in our repo).]{.underline}
...
-   [Task 5: Annotating tables, has been performed inline, in a more
    ...
    provided images and tables adjacently to each.]{.underline}

-   ⭐️ [Stars denote items corresponding directly to the
    rubric]{.underline}
...
1.  Unsupervised Analysis
    ...
    d.  How many clusters did each method find?
        a.  If you ran a method that requires you to select the number
            of clusters (k), how did changing this value change the
            results? Compare cluster membership at each k to investigate
            this.
        b.  If you ran a method that selects k for you, describe why it
            chose that k.
    e.  Rerun each clustering method using different numbers of genes.
        Try 10, 100, 1000, and 10000 genes.
        a.  How did the number of genes affect clustering?
        b.  Create an alluvial diagram (Sankey plot) to visualize how
            ...
            ([https://cran.r](https://cran.r-)project.org/web/packages/ggalluvial/vignettes/ggalluvial.html)
...
#set.seed(123)
#below line takes wayyy too long, i took a picture so maybe we can insert that and say we took it out in interest of time
#fviz_nbclust(top_5000, kmeans, method = "wss")

# using the elbow method, the optimal number of clusters appears to be 4
```

```{r}
k_three <- kmeans(top_5000,3,iter.max = 10, nstart = 25)
k_four <- kmeans(top_5000,4,iter.max = 10, nstart = 25)
k_five <- kmeans(top_5000,5,iter.max = 10, nstart = 25)
print(k_three)
print(k_four)
print(k_five)
```
...
```{r}
# The below command is supposed to show the data in a cool graph with groupings, but I can't get it to work because R says some file doesn't exist?
#fviz_cluster(k_four, data = top_5000)
```

# 2D K-MEANS - i.	If you ran a method that requires you to select the number of clusters (k), how did changing this value change the results? Compare cluster membership at each k to investigate this.

The K-means clustering method required me to select the number of clusters. To determine the ideal "k", I utilized the "factoextra" package to generate a graph showing the number of clusters versus the total within the sum of square. Using the elbow method, I determined that the optimal number of cluster was 4, and to test how changing the value changed the results, ran the "kmeans" clustering command for a k of 3,4, and 5 to determine how changing k changed the results.
...
By assigning k of 3 as opposed to 5000, it achieved of a reduction in sum of squares by 32.3%. At a k of 4, this rose greatly to 37.3% At a k of 5, it rose only slightly to 39.9%. By looking at cluster membership at k = 3,4,5, we can see that cluster membership does not change drastically between 4 and 5. Between those 2 points, the top 3 largest groups have largely the same numbers. Meanwhile, between a k of 3 and 4, the top 3 largest groups have drastically different numbers. These data together confirms the earlier elbow method, telling us that 4 is the most ideal number of clusters.

# Sahas - Hierarchical Clustering (hclust)

[Note, \~7 minute runtime on Macbook Air M2 (known for CPU), expect
delay.]{.underline}
...
# 1 cluster
hcluster1 <- cutree(hcluster, k = 1)
# 2 clusters
hcluster2 <- cutree(hcluster, k = 2)
# 3 clusters 
hcluster3 <- cutree(hcluster, k = 3)
...
# Visualize clusters
fviz_cluster(list(data = top_5000, cluster = hcluster2), geom = ("point"))
...
plot(hclusterTop10, cex = 0.8, hang = -1)
hclusterCut10 <- cutree(hclusterTop10, k = 2)
...
plot(hclusterTop100, cex = 0.8, hang = -1)
hclusterCut100 <- cutree(hclusterTop100, k = 2)
...
plot(hclusterTop1000, cex = 0.8, hang = -1)
hclusterCut1000 <- cutree(hclusterTop1000, k = 2)
...
plot(hclusterTop10000, cex = 0.8, hang = -1)
hclusterCut10000 <- cutree(hclusterTop10000, k = 2)
...
k <- 3
pam_result_top10 <- pam(top_10m, k = k, diss = TRUE)
pam_result_top100 <- pam(top_100m, k = k, diss = TRUE)
pam_result_top1000 <- pam(top_1000m, k = k, diss = TRUE)
pam_result_top5000 <- pam(top_5000m, k = k, diss = TRUE)
pam_result_top10000 <- pam(top_10000m, k = k, diss = TRUE)
```
...
k <- 3
pam_result_top5000 <- pam(top_5000m, k = k, diss = TRUE)
cluster_counts <- table(pam_result_top5000$clustering)
cluster_counts
...
[I spent over an hour attempting to generate a nice sankey diagram of
the transitions of genes between clusters but was unable to do so and
have concluded that even if I did, the information would not be
overwhelmingly insightful. Point is, we can see that increasing the
cluster count discriminates our top 5000 genes into additional groups.
To evaluate "goodness of clustering" we can use the silhouette method,
which is comparable to agglomerative clustering in how it measures
goodness as a function of not only proximity of points within clusters
but of the distance between clusters. By graphing the sillhouette score
for different K values, we can identify an optimal K. I learned of
silhouette in the PAM instructions and while investigating goodness of
clustering methods for PAM.]{.underline}
...
#should identify 2 as optimal K value
```

```{r}
pam_result_top5000 <- pam(top_5000m, k = 2, diss = TRUE)
fviz_silhouette(pam_result_top5000, label=TRUE)
```
#3E 
```{r}
# rerun K-Means clustering method with different number of genes
k_ten <- kmeans(top_10,4,iter.max = 10,nstart=25)
print(k_ten)
k_hundred <- kmeans(top_100,4,iter.max = 10,nstart=25)
print(k_hundred)
k_k <- kmeans(top_1000,4,iter.max = 10,nstart=25)
print(k_k)
k_tenk <- kmeans(top_10000,4,iter.max = 10,nstart=25)
print(k_tenk)

``` 
#3E.A - How did the number of genes affect clustering?
...
Looking solely at cluster membership, it appears that each time the number of genes increased by 10 times, membership in the 1,2nd, and 3rd largest groups roughly followed the magnification as well.
As the number of genes increased, a reduction in sum of squares at a k of 4 was calculated as 70.5%, 36.8%, 35.5,% and 39.1%. 


#3E.B - Alluvial Diagram of K-Means CLustering
```{r}
if(!require("ggplot2"))
...
#an alluvial diagram needs to be created for EACH of the clustering methods for 10, 100, 1000, and 10000 genes
copy_ten <- top_10
copy_ten$cluster <- k_ten$cluster
#copy_ten
is_alluvia_form(copy_ten)
copy_ten <- copy_ten %>%
  tibble::rownames_to_column("Gene")

ggplot(data = copy_ten,
       aes(axis1 = Gene, axis2 = cluster, 
           y = row_variance)) +
  scale_x_discrete(limits = c("Gene", "Cluster", "row_variance"), expand = c(.2, .05)) +
  xlab("Gene Clusters") +
  geom_alluvium(aes(fill = cluster)) +
  geom_stratum() +
  geom_text(stat = "stratum", aes(label = after_stat(stratum))) +
  theme_minimal() +
  ggtitle("10 Genes with 4 Clusters (K-means)")
...
#an alluvial diagram needs to be created for EACH of the clustering methods for 10, 100, 1000, and 10000 genes
copy_100 <- top_100
copy_100$cluster <- k_ten$cluster
#copy_ten
is_alluvia_form(copy_100)
copy_100 <- copy_100 %>%
  tibble::rownames_to_column("Gene")

ggplot(data = copy_100,
       aes(axis1 = Gene, axis2 = cluster, 
           y = row_variance)) +
  scale_x_discrete(limits = c("Gene", "Cluster", "row_variance"), expand = c(.2, .05)) +
  xlab("Gene Clusters") +
  ...
  geom_text(stat = "stratum", aes(label = after_stat(stratum))) +
  theme_minimal() +
  ggtitle("100 Genes with 4 Clusters (K-means)")
...
#an alluvial diagram needs to be created for EACH of the clustering methods for 10, 100, 1000, and 10000 genes
copy_1000 <- top_1000
copy_1000$cluster <- k_ten$cluster
#copy_ten
is_alluvia_form(copy_1000)
copy_1000 <- copy_1000 %>%
  tibble::rownames_to_column("Gene")

ggplot(data = copy_1000,
       aes(axis1 = Gene, axis2 = cluster, 
           y = row_variance)) +
  scale_x_discrete(limits = c("Gene", "Cluster", "row_variance"), expand = c(.2, .05)) +
  xlab("Gene Clusters") +
  ...
  geom_text(stat = "stratum", aes(label = after_stat(stratum))) +
  theme_minimal() +
  ggtitle("1,000 Genes with 4 Clusters (K-means)")
```
```{r}
#an alluvial diagram needs to be created for EACH of the clustering methods for 10, 100, 1000, and 10000 genes
copy_10000 <- top_10000
copy_10000$cluster <- k_ten$cluster
#copy_ten
is_alluvia_form(copy_10000)
copy_10000 <- copy_10000 %>%
  tibble::rownames_to_column("Gene")

ggplot(data = copy_10000,
       aes(axis1 = Gene, axis2 = cluster, 
           y = row_variance)) +
  scale_x_discrete(limits = c("Gene", "Cluster", "row_variance"), expand = c(.2, .05)) +
  xlab("Gene Clusters") +
  ...
  geom_text(stat = "stratum", aes(label = after_stat(stratum))) +
  theme_minimal() +
  ggtitle("10,000 Genes with 4 Clusters (K-means)")
```
...
#for this, we need to also have the sample groupings (diabetic, non-diabetic AND pancreatic islets because we have to have all 5,000 genes)
#i am confused
```{r}
...
### 🚨🚨🚨🚨🚨🚨🚨🚨🚨Everything Below this Point is Assignment 2 (For reference) 🚨🚨🚨🚨🚨🚨🚨🚨🚨
