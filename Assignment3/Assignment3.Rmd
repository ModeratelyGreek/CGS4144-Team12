...
#2B
```{r INCLUDE=FALSE}
# compute K-means clustering for top 5000 genes at k = 1,2,5,10
# 

kmeans(top_5000,1,iter.max = 10,nstart=1)

kmeans(top_5000,2,iter.max = 10,nstart=1)

kmeans(top_5000,5,iter.max = 10,nstart=1)

kmeans(top_5000,10,iter.max = 10,nstart=1)
...
The K-means clustering method required me to select the number of clusters. To determine the impact changing k has on the results, I ran the method for four different values (1,2,5, and 10) and obtained the following cluster membership values: 

From this data, we can see that increasing the number of clusters also increases cluster membership, indicating that our data is quite spread out rather than concentrated.

#3E 
```{r include=FALSE}
# rerun K-Means clustering method with different number of genes
k_ten <- kmeans(top_10,5,iter.max = 10,nstart=1)
k_hundred <- kmeans(top_100,5,iter.max = 10,nstart=1)
k_k <- kmeans(top_1000,5,iter.max = 10,nstart=1)
k_tenk <- kmeans(top_10000,5,iter.max = 10,nstart=1)
...
# data cannot be a list like what kmeans outputs?
is_alluvia_form(top_5000)
...
x = data.matrix(top_5000, rownames.force = NA)
ht = ComplexHeatmap::Heatmap(x,heatmap_legend_param = list(
        title = "5,000 Genes used for K-Means Clustering"
    ),column_title = "Clusters",show_row_names = FALSE, show_column_names = FALSE,cluster_columns = FALSE,cluster_rows = TRUE)
