...
-   ⭐️ Stars denote items corresponding directly to the rubric
...
# using the elbow method, the optimal number of clusters appears to be 4 (since this is around where the curve begins to flatten out)
```
...
```{r}
fviz_cluster(kmeans_result_5000_five, data = top_5000)
```
...
that the optimal number of cluster was 4, and to test how changing the
...
scale_x_discrete(
  limits = c("Gene", "Cluster", "row_variance"), 
  expand = c(.2, .05)
) 
